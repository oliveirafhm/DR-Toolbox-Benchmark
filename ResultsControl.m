...
% Insert alert that this script uses vars from anothers scripts
% workFolder
resultsControlPath = [workFolder '/' 'ResultsControl.xlsx'];
...
for i = 1:length(groupExp)
    ...
    % Cross validation general mean of SPD(SPD)
    overviewTempSheet{5} = resultsSumRange{1,5};
    % Cross validation general mean of SDBS(SDBS)
    ...
    % Test general mean of SPD(SPD)
    overviewTempSheet{8} = resultsSumRange{1,5};
    % Test general mean of SDBS(SDBS)
    overviewTempSheet{9} = resultsSumRange{1,9};
    for ii = 1:length(kk)
