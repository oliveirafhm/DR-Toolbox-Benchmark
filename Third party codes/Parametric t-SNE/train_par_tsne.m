function [network, err] = train_par_tsne(train_X, train_labels, test_X, test_labels, layers, training, max_iter, perplexity)
...
% Maastricht University, 2008
    ...
    for i=1:no_layers
        ...
        if i ~= no_layers
            ...
            if strcmp(training, 'CD1')
                network{i} = train_rbm(train_X, layers(i));%, 0.1, max_iter); % @oliveirafhm mod
            elseif strcmp(training, 'PCD')
                network{i} = train_rbm_pcd(train_X, layers(i));%, 0.02, max_iter); % @oliveirafhm mod
            ...
            if ~strcmp(training, 'None')
                network{i} = train_lin_rbm(train_X, layers(i));%, 0.001, max_iter);
            else
                v = size(train_X, 2);
                network{i}.W = randn(v, layers(i)) * 0.1;
                network{i}.bias_upW = zeros(1, layers(i));
    ...
    [network, err] = tsne_backprop(network(1:no_layers), origX, train_labels, test_X, test_labels, ...
        max_iter, perplexity, 1);
