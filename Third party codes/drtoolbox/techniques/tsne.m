function [ydata, cost] = tsne(X, labels, no_dims, initial_dims, perplexity, max_iter, learning_rate)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if ~exist('learning_rate', 'var') || isempty(learning_rate)
        learning_rate = 500;
    end
    ...
    % Run t-SNE (@oliveirafhm mod)
    if initial_solution
        [ydata, cost] = tsne_p(P, labels, ydata, max_iter, learning_rate);
    else
        [ydata, cost] = tsne_p(P, labels, no_dims, max_iter, learning_rate);
    end
