...
%     end
    ...
    switch type
        ...
        case {'tSNE', 't-SNE'}
			...
			if isempty(varargin), [mappedA, mapping.error] = tsne(A, [], no_dims);
            else [mappedA, mapping.error] = tsne(A, varargin{5}, no_dims, varargin{1}, varargin{2}, varargin{3}, varargin{4}); end
            mapping.name = 't-SNE';
...
%             [mappedA, network, binary_data, mean_X, var_X] = train_autoencoder(A, net_structure);
        ...
        case 'Sammon'
            %@oliveirafhm mod
            if isempty(varargin), [mappedA, mapping.error, mapping.iter]...
                    = sammon(A, no_dims);
            else [mappedA, mapping.error, mapping.iter]...
                    = sammon(A, no_dims, varargin{1});end
            mapping.name = 'Sammon';
