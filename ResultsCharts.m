...
%     idx = find([resultsControlData{:,2}] == j);
    if j ~= 3 
        idx = find([resultsControlData{:,2}] == j);
    % Pick perplexity value to filter
    else        
    ...
    for k = 1:nBars
        ...
        if k <= nBars / 2
            if exist('GM_CV','var')
                GM_CV(end+1) = {[resultsControlData{idx,GMCols(kk)}]};
            else
                GM_CV = {[resultsControlData{idx,GMCols(kk)}]};
            end
        else
            if exist('GM_T','var')
                GM_T(end+1) = {[resultsControlData{idx,GMCols(kk)}]};
            else
...
%         if exist('GM','var')
%             GM(end+1) = {[resultsControlData{idx,GMCols(kk)}]};
%         else            
%             GM = {[resultsControlData{idx,GMCols(kk)}]};
%         end
        kk = kk + 2;
    end
end
